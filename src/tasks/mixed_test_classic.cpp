...
AnalyticSolution buildAnalytic(const VariantData& variant, const TestCoefficients& tc) {
    ...
    m[1] = {
        0.0,
        0.0,
        tc.kRight * s.lambdaRight * eR1,
        -tc.kRight * s.lambdaRight * eRm1,
        variant.mu2
    };
...
std::vector<double> solveGrid(int n, const VariantData& variant, const TestCoefficients& tc) {
    ...
    std::vector<double> diag(static_cast<size_t>(size), 0.0);
    ...
    for (int i = 1; i <= n - 1; ++i) {
        ...
        const double invKi = splitIntegral(xi - h, xi, variant.xi, 1.0 / tc.kLeft, 1.0 / tc.kRight);
        const double invKip1 = splitIntegral(xi, xi + h, variant.xi, 1.0 / tc.kLeft, 1.0 / tc.kRight);

        const double ai = h / invKi;
        const double aip1 = h / invKip1;
        ...
        lower[static_cast<size_t>(i - 1)] = -ai / h2;
        diag[static_cast<size_t>(i)] = (ai + aip1) / h2 + di;
        upper[static_cast<size_t>(i)] = -aip1 / h2;
        rhs[static_cast<size_t>(i)] = phi;
    }

    // Classical approximation of k(1)u'(1)=mu2: k(1)*(v_n-v_{n-1})/h = mu2.
    lower[static_cast<size_t>(n - 1)] = -tc.kRight / h;
    diag[static_cast<size_t>(n)] = tc.kRight / h;
...
TaskResult runMixedTestClassicTask(const InputData& input, const VariantData& variant) {
    ...
    note << "Смешанная тестовая задача (классическая аппроксимация ГУ).\n"
         << "Для решения задачи использована равномерная сетка с числом разбиений n = " << n << ";\n"
